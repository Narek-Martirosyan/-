...
export const todoAppSlice = createSlice({
    ...
    reducers: {
        ...
        editTodo: (state, action) => {
            state.list[action.payload.initialIndex].title = action.payload.result;
        },
        ...
        checkboxChecked: (state, action) => {
            state.list[action.payload.index].completed = action.payload.checked;
            if (state.list[action.payload.index].completed) {
                state.num += 1;
            } else {
                state.num -= 1;
            }
